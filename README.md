...
| **Reactions** (polled, NOT event-driven) | Telegram does **not** push `updateMessageReactions` to user accounts for 1-on-1 private chats (server-side restriction — the same asymmetry the Bot API documents). No event handler can catch them, even a catch-all `events.Raw`. We work around this by polling: a background task (`start_reaction_polling` in [handlers.py](handlers.py)) runs every 10 s, fetches every relay-mapped message recorded in the last 24 h via `client.get_messages(chat, ids=[...])` (one API call per chat per cycle), pulls `m.reactions`, filters out our own reactions via `MessagePeerReaction.my` / `ReactionCount.chosen_order`, and mirrors the partner's set onto the counterpart via raw `SendReactionRequest`. In-memory state tracks the last-seen reaction signature per `(chat_id, msg_id)` so unchanged reactions don't re-mirror every cycle. State is lost on restart — the first poll after a restart re-mirrors everything (idempotent: `SendReactionRequest` with the current reaction set is a no-op on Telegram's side). |
